...
import { UploadAttributeMasterFile } from '../../../store/actions';
import { DashboardState } from '../../../store/reducers';
import { getAttributeFileUploadLoading } from '../../../store/selectors';

@Component({
  selector: 'app-attribute-master-dialog',
  templateUrl: './attribute-master-dialog.component.html',
  styleUrls: ['./attribute-master-dialog.component.scss']
})
export class AttributeMasterDialogComponent implements OnInit, OnDestroy {
  @ViewChild('fileUpload') fileUpload: ElementRef;
  loading$: Observable<any> = this.store.select(getAttributeFileUploadLoading);
  file: File;
  ...
  onSubmit(): void {
    ...
    this.store.dispatch(new UploadAttributeMasterFile(formData));
  }
  ...
  private validateNaming(name: string): boolean {
    return new RegExp('^HIDA_Attribute_Master_UP_[a-z]{1,8}$', 'gi').test(name.split('.')[0]);
  }
