...
import { UploadBrandSuggestionFileUpload } from '../../../store/actions';
import { DashboardState } from '../../../store/reducers';
import { getBrandFeatureSuggestionFileLoading } from '../../../store/selectors';
import * as moment from 'moment';


@Component({
  selector: 'app-brand-feature-suggestion-dialog',
  templateUrl: './brand-feature-suggestion-dialog.component.html',
  styleUrls: ['./brand-feature-suggestion-dialog.component.scss']
})
export class BrandFeatureSuggestionDialogComponent implements OnInit, OnDestroy {
  @ViewChild('fileUpload') fileUpload: ElementRef;
  loading$: Observable<any> = this.store.select(getBrandFeatureSuggestionFileLoading);
  file: File;
  ...
  onSubmit(): void {
    ...
    this.store.dispatch(new UploadBrandSuggestionFileUpload(formData));
  }
  ...
  private validateNaming(name: string): boolean {
    return new RegExp(`^HIDA_Brand_Suggestion_${this.env}$`, 'gi').test(name.split('.')[0]);
  }
