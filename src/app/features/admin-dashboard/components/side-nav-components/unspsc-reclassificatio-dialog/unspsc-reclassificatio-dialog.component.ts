...
import { UploadUnspscReclassificationFile } from '../../../store/actions';
import { DashboardState } from '../../../store/reducers';
import { getReclassifiedFileUploadLoading } from '../../../store/selectors';

@Component({
  selector: 'app-unspsc-reclassificatio-dialog',
  templateUrl: './unspsc-reclassificatio-dialog.component.html',
  styleUrls: ['./unspsc-reclassificatio-dialog.component.scss']
})
export class UnspscReclassificatioDialogComponent implements OnInit, OnDestroy {
  @ViewChild('fileUpload') fileUpload: ElementRef;
  loading$: Observable<any> = this.store.select(getReclassifiedFileUploadLoading);
  file: File;
  ...
  onSubmit(): void {
    ...
    this.store.dispatch(new UploadUnspscReclassificationFile(formData));
  }
  ...
  private validateNaming(name: string): boolean {
    return new RegExp('^HIDA_UNSPSC_RECLASSIFICATION_[0-9]{8}_[0-9]{1,8}$', 'gi').test(name.split('.')[0]); // for ref: HIDA_UNSPSC_RECLASSIFICATION_<8-digit-UNSPSC>_<yyyyMMdd>.csv
  }
