...
import { ManufacturerParentChildUploadFile } from '../../../store/actions';
import { DashboardState } from '../../../store/reducers';
import { getManufacturerParentChildFileUploadLoading } from '../../../store/selectors';

@Component({
  selector: 'app-manufacturer-parent-child-dialog',
  templateUrl: './manufacturer-parent-child-dialog.component.html',
  styleUrls: ['./manufacturer-parent-child-dialog.component.scss']
})
export class ManufacturerParentChildDialogComponent implements OnInit, OnDestroy {
  @ViewChild('fileUpload') fileUpload: ElementRef;
  loading$: Observable<any> = this.store.select(getManufacturerParentChildFileUploadLoading);
  file: File;
  ...
  onSubmit(): void {
    ...
    this.store.dispatch(new ManufacturerParentChildUploadFile(formData));
  }
  ...
  private validateNaming(name: string): boolean {
    return new RegExp('^HIDA_ManufacturerParentChild_UP_[a-z]{1,8}$', 'gi').test(name.split('.')[0]);
  }
