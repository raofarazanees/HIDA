...
import { ManufacturerCVsUploadFile } from '../../../store/actions';
import { DashboardState } from '../../../store/reducers';
import { getManufacturerCVsFileUploadLoading } from '../../../store/selectors';

@Component({
  selector: 'app-ontology-manufacturer-dialog',
  templateUrl: './ontology-manufacturer-dialog.component.html',
  styleUrls: ['./ontology-manufacturer-dialog.component.scss']
})
export class OntologyManufacturerDialogComponent implements OnInit, OnDestroy {
  @ViewChild('fileUpload') fileUpload: ElementRef;
  loading$: Observable<any> = this.store.select(getManufacturerCVsFileUploadLoading);
  file: File;
  ...
  onSubmit(): void {
    ...
    this.store.dispatch(new ManufacturerCVsUploadFile(formData));
  }
  ...
  private validateNaming(name: string): boolean {
    return new RegExp('^HIDA_Manufacturer_UP_[a-z]{1,8}$', 'gi').test(name.split('.')[0]);
  }
