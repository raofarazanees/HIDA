...
import { FacilityTypeCVsUploadFile } from '../../../store/actions';
import { DashboardState } from '../../../store/reducers';
import { getFacilityTypeCVsFileUploadLoading } from '../../../store/selectors';

@Component({
  selector: 'app-ontology-failitytype-dialog',
  templateUrl: './ontology-failitytype-dialog.component.html',
  styleUrls: ['./ontology-failitytype-dialog.component.scss']
})
export class OntologyFacilityTypeDialogComponent implements OnInit, OnDestroy {
  @ViewChild('fileUpload') fileUpload: ElementRef;
  loading$: Observable<any> = this.store.select(getFacilityTypeCVsFileUploadLoading);
  file: File;
  ...
  onSubmit(): void {
    ...
    this.store.dispatch(new FacilityTypeCVsUploadFile(formData));
  }
  ...
  private validateNaming(name: string): boolean {
    return new RegExp('^HIDA_FacilityType_UP_[a-z]{1,8}$', 'gi').test(name.split('.')[0]);
  }
